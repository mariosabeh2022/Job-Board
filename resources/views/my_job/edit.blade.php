<x-layout>
    ...
    <x-card class="mb-8">
        <form action="{{ route('my-jobs.update', $job) }}" method="POST"> @csrf
            @method('PUT')
            <div class="mb-4 grid grid-cols-2 gap-4">
                ...
                <div class="col-span-2">
                    ...
                    <x-text-input name="salary" :value="$job->salary" type="number" />
                </div>


                <div class="col-span-2">
                    <x-label for="description" :required="true">Description</x-label>
                    <x-text-input name="description" :value="$job->description" type="textarea" />
                </div>

                <div>
                    <x-label for="experience" :required="true">Experience</x-label>
                    <x-radio-group :allOption="false" name="experience" :value="old('experience', $job->experience)"
                        :options="array_combine(array_map('ucfirst', \App\Models\Job::$experience), \App\Models\Job::$experience)" />
                </div>

                <div>
                    <x-label for="category" :required="true">Category</x-label>
                    <x-radio-group :allOption="false" name="category" :value="old('category', $job->category)"
